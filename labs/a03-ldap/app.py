...
# Mock Active Directory LDAP database
users = [
    {
        "cn": "admin_sec_operator",
        "description": "FLAG{ldap_filter_wildcard_leak}",
        "mail": "admin@corp.local",
        "memberOf": "Domain Admins"
    },
    {
        "cn": "guest_user",
        "description": "Standard read-only account",
        "mail": "guest@corp.local",
        "memberOf": "Domain Guests"
    }
]

# Security settings & logs API
...
<head>
  ...
  <title>LDAP Injection Security Laboratory</title>
...
<body>
  ...
  <header>
    <div class="brand">
      ...
      LDAP Injection Security Laboratory
    </div>
    ...
    <div class="panel">
      ...
      <div class="stages-grid">
        <div class="stage-card active" onclick="selectStage(1)">
          ...
          <div class="stage-name">LDAP Filter</div>
        ...
        <div class="stage-card" onclick="selectStage(2)">
          ...
          <div class="stage-name">Wildcard Block</div>
        ...
        <div class="stage-card" onclick="selectStage(3)">
          ...
          <div class="stage-name">LDAP Timing</div>
        ...
        <div class="stage-card" onclick="selectStage(4)">
          ...
          <div class="stage-name">Attr Injection</div>
          <div class="stage-status unsolved" id="status-4">UNSOLVED</div>
        </div>
      </div>

      <div class="desc-box">
        <div class="desc-title" id="stageTitle">Stage 1: Blind LDAP search character extraction</div>
        <div class="desc-body" id="stageDesc">
          LDAP queries are formed by direct concatenation. Exfiltrate the administration credentials character-by-character using nested filters and wildcards.
        </div>
      </div>

      <div class="field-group">
        <label>LDAP Query Search parameter</label>
        <input type="text" id="queryInput" value="admin_sec_operator)(description=FLAG{l*">
      </div>

      <button class="btn" onclick="executeLdapSearch()">Dispatch LDAP Search</button>

      <div class="field-group" style="margin-top: 1rem;">
        <label>LDAP Response Outputs</label>
  ...
  <script>
    ...
    const stageDetails = {
      1: {
        title: "Stage 1: Blind LDAP search character extraction",
        desc: "Wildcard queries allow enumerating attribute values. Extract using asterisk matches: `admin_sec_operator)(description=FLAG{l*`.",
        defaultVal: "admin_sec_operator)(description=FLAG{l*"
      },
      2: {
        title: "Stage 2: LDAP Wildcard Filter WAF Bypass",
        desc: "The WAF screens for wildcard characters `*`. Solve by using LDAP OR query groups or dynamic null byte character truncations.",
        defaultVal: "admin_sec_operator)(description=FLAG{ldap_filter_wildcard_leak"
      },
      3: {
        title: "Stage 3: LDAP Timing Side-Channel Attack",
        desc: "Timing side-channel exfiltration on LDAP query parameters. Inject time delays dynamically inside matching filter conditions.",
        defaultVal: "admin_sec_operator"
      },
      4: {
        title: "Stage 4: Administrative Attribute Injection",
        desc: "Inject attributes to overwrite session properties. Exploit to grant administrative permissions.",
        defaultVal: "admin_sec_operator"
      }
    };
    ...
    async function executeLdapSearch() {
      ...
      consoleOut.value = `Dispatching LDAP search query...\n`;
      
      try {
        const res = await fetch(`/api/search?query=${encodeURIComponent(query)}&stage=${currentStage}`);
        const data = await res.json();
        consoleOut.value = JSON.stringify(data, null, 2);

        if (data.success && data.results && data.results.length > 0) {
          localStorage.setItem(`stage_ldap_${currentStage}_solved`, 'true');
          updateSolvedBadges();
        }
      } catch (err) {
        consoleOut.value = `LDAP Search Error: ${err.message}`;
      }
    }

    function updateSolvedBadges() {
      for (let i = 1; i <= 4; i++) {
        const solved = localStorage.getItem(`stage_ldap_${i}_solved`) === 'true';
...
# A simple LDAP filter evaluator to mock typical Wildcard search matching
def match_filter(user, filter_str):
    filter_str = filter_str.replace(" ", "")
    
    if filter_str.startswith("(&"):
        sub_filters = parse_sub_filters(filter_str[2:-1])
        return all(match_filter(user, f) for f in sub_filters)
    elif filter_str.startswith("(|"):
        sub_filters = parse_sub_filters(filter_str[2:-1])
        return any(match_filter(user, f) for f in sub_filters)
    elif filter_str.startswith("(!"):
        sub_filter = filter_str[2:-1]
        return not match_filter(user, sub_filter)
    
    if filter_str.startswith("(") and filter_str.endswith(")"):
        content = filter_str[1:-1]
        if "=" in content:
            attr, val = content.split("=", 1)
            if attr in user:
                user_val = user[attr]
                if val == "*":
                    return True
                    ...
                    prefix = val[:-1]
                    return user_val.startswith(prefix)
                elif val.startswith("*"):
                    suffix = val[1:]
                    return user_val.endswith(suffix)
                else:
                    return user_val == val
    return False

def parse_sub_filters(content):
    groups = []
    stack = 0
    current = ""
    for char in content:
...
def search():
    ...
    if security_mode == 'secure':
        # SECURE: Strict input sanitization blocking LDAP filter characters
        query_input = query_input.replace("(", "").replace(")", "").replace("&", "").replace("|", "").replace("*", "")
    
    ldap_filter = f"(&(cn={query_input})(memberOf=Domain Admins))"
    
    matched_results = []
    for user in users:
        try:
            if match_filter(user, ldap_filter):
            ...
            add_log('GET', '/api/search', 500)
            return jsonify({"success": False, "error": f"LDAP parser error: {str(e)}"})
