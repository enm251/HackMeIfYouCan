...
# XML Catalog representation in memory
catalog = [
    {"name": "Standard Secure Widget", "description": "High-grade commercial diagnostic connector", "type": "public"},
    {"name": "Developer Beta Interface", "description": "Internal testing debug bridge", "type": "internal"},
    {"name": "Platform Secret Flag Node", "description": "FLAG{xpath_xml_traversal_success}", "type": "confidential"}
]

stage_flags = {
    2: "FLAG{xpath_waf_boolean_bypass}",
    3: "FLAG{xpath_blind_brute_force}",
    4: "FLAG{xpath_xxe_chain_expert}"
}
...
<head>
  ...
  <title>XPath Injection Security Laboratory</title>
...
<body>
  ...
  <header>
    <div class="brand">
      ...
      XPath Injection Security Laboratory
    </div>
    ...
    <div class="panel">
      ...
      <div class="stages-grid">
        <div class="stage-card active" onclick="selectStage(1)">
          ...
          <div class="stage-name">XPath Tautology</div>
        ...
        <div class="stage-card" onclick="selectStage(2)">
          ...
          <div class="stage-name">Boolean WAF</div>
        ...
        <div class="stage-card" onclick="selectStage(3)">
          ...
          <div class="stage-name">Blind XPath</div>
        ...
        <div class="stage-card" onclick="selectStage(4)">
          ...
          <div class="stage-name">XML Entity</div>
          <div class="stage-status unsolved" id="status-4">UNSOLVED</div>
        </div>
      </div>

      <div class="desc-box">
        <div class="desc-title" id="stageTitle">Stage 1: XPath Tautology Injection</div>
        <div class="desc-body" id="stageDesc">
          Concatenated arguments inside XML queries allow listing all confidential nodes. Input `1' or true() or '` to trigger a tautology match.
        </div>
      </div>

      <div class="field-group">
        <label>Catalog Item Search Query</label>
        <input type="text" id="queryInput" value="1' or true() or '">
      </div>

      <button class="btn" onclick="executeXPathSearch()">Dispatch XPath Search</button>

      <div class="field-group" style="margin-top: 1rem;">
        <label>XPath Query Engine Output</label>
  ...
  <script>
    ...
    const stageDetails = {
      1: {
        title: "Stage 1: XPath Tautology Authentication Bypass",
        desc: "The search directly appends input inside standard xpath selectors. Bypass by supplying `1' or true() or '` to dump confidential flags.",
        defaultVal: "1' or true() or '"
      },
      2: {
        title: "Stage 2: Boolean WAF Keyword Filtering",
        desc: "The WAF checks for literal keywords 'or', 'and', 'true'. Bypass the check by using boolean arithmetic operations or parent tree lookups.",
        defaultVal: "1' or '1'='1"
      },
      3: {
        title: "Stage 3: Blind XPath character brute-force",
        desc: "Blind timing exfiltration over XML document attributes. Exfiltrate the confidential values pos-by-pos.",
        defaultVal: "1"
      },
      4: {
        title: "Stage 4: Dynamic XML Entity References (XXE)",
        desc: "XPath queries trigger dynamic entity mapping. Chain vectors to exfiltrate sensitive files.",
        defaultVal: "1"
      }
    };
    ...
    function selectStage(num) {
      ...
      document.getElementById('queryInput').value = stageDetails[num].defaultVal;
    }
    ...
    async function executeXPathSearch() {
      const query = document.getElementById('queryInput').value;
      const consoleOut = document.getElementById('consoleOutput');
      consoleOut.value = `Dispatching XPath query search...\n`;
      
      try {
        const res = await fetch(`/api/search?name=${encodeURIComponent(query)}&stage=${currentStage}`);
        const data = await res.json();
        consoleOut.value = JSON.stringify(data, null, 2);

        if (data.success && data.results) {
          const flagNode = data.results.find(r => r.name === 'Platform Secret Flag Node');
          if (flagNode) {
            localStorage.setItem(`stage_xpath_${currentStage}_solved`, 'true');
            updateSolvedBadges();
          }
        }
      } catch (err) {
        consoleOut.value = `XPath Search Error: ${err.message}`;
      }
    }

    function updateSolvedBadges() {
      for (let i = 1; i <= 4; i++) {
        const solved = localStorage.getItem(`stage_xpath_${i}_solved`) === 'true';
...
def search():
    query_input = request.args.get("name", "")
    stage = int(request.args.get("stage", "1"))
    if not query_input:
        add_log('GET', '/api/search', 400)
        return jsonify({"success": False, "error": "Search query parameter required"})
    
    if security_mode == 'secure':
        # SECURE: Strict parameter validation, escape single quotes
        query_input = query_input.replace("'", "''").replace('"', '""')
    
    query_normalized = query_input.strip()
    matched = []
    is_tautology = False
    
    if security_mode == 'vulnerable':
        # WAF check for Stage 2
        if stage == 2:
            lowered = query_normalized.toLowerCase() if hasattr(query_normalized, 'toLowerCase') else query_normalized.lower()
            if "or" in lowered or "and" in lowered or "true" in lowered:
                add_log('GET', '/api/search', 403)
                return jsonify({"success": False, "error": "IPS BLOCK: Dangerous boolean XPath keywords restricted."})

        # Evaluate tautologies
        if ("or true()" in query_normalized or 
            "or 1=1" in query_normalized or 
            "'=''" in query_normalized or
            "'='" in query_normalized or
            "|" in query_normalized or
            "or '1'='1'" in query_normalized):
            is_tautology = True
            
    if is_tautology:
        # Clone catalog and map flags based on stage
        custom_catalog = []
        for p in catalog:
            item = p.copy()
            if item["name"] == "Platform Secret Flag Node":
                if stage in stage_flags:
                    item["description"] = stage_flags[stage]
            custom_catalog.append(item)
        matched = custom_catalog
    else:
        matched = [p for p in catalog if p["name"].lower() == query_normalized.lower()]
        
    add_log('GET', '/api/search', 200)
    return jsonify({
        "success": True,
        "results": matched
    })
